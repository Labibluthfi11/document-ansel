<x-app-layout>
    <div class="py-10 bg-gray-50 min-h-screen">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-8">
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full border border-gray-200 rounded-2xl overflow-hidden shadow">
                        <thead class="bg-gradient-to-r from-blue-100 to-purple-100">
                            <tr>
                                <th class="py-3 px-4 font-bold text-gray-700 border-b">#</th>
                                <th class="py-3 px-4 font-bold text-gray-700 border-b">Nama</th>
                                <th class="py-3 px-4 font-bold text-gray-700 border-b">Email</th>
                                <th class="py-3 px-4 font-bold text-gray-700 border-b">Role</th>
                                <th class="py-3 px-4 font-bold text-gray-700 border-b">Aksi</th>
                            </tr>
                        </thead>
                        <tbody>
                                            ...
                                            <div class="flex flex-wrap items-center gap-2">
                                                ...
                                                @if($user->role !== 'admin')
                                                    <form method="POST" action="{{ route('admin.users.makeAdmin', $user->id) }}">
                                                        @csrf
                                                        <button type="submit" class="flex items-center gap-1 px-3 py-1 bg-purple-600 text-white rounded-xl hover:bg-purple-700 transition font-medium shadow-sm">
                                                ...
                                                @else
                                                    <form method="POST" action="{{ route('admin.users.makeUser', $user->id) }}">
                                                        @csrf
                                                        <button type="submit" class="flex items-center gap-1 px-3 py-1 bg-gray-500 text-white rounded-xl hover:bg-gray-700 transition font-medium shadow-sm">
                                                ...
                                                <form method="POST" action="{{ route('admin.sendResetPassword', $user->id) }}">
                                                    @csrf
                                                    <button type="submit" class="flex items-center gap-1 px-3 py-1 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition font-medium shadow-sm">
                                                ...
                                                <form method="POST" action="{{ route('admin.users.destroy', $user->id) }}" onsubmit="return confirm('Yakin ingin menghapus user ini?');">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="flex items-center gap-1 px-3 py-1 bg-red-600 text-white rounded-xl hover:bg-red-700 transition font-medium shadow-sm">
            ...
            <a href="{{ route('dashboard') }}" class="inline-flex items-center mt-6 px-6 py-3 bg-blue-600 text-white rounded-xl font-semibold shadow hover:bg-blue-700 transition">
